...
OpenSslTestKeyProvider::resolve_root(RootKeyReference reference) noexcept {
    ...
    if (token == 0U || generation == 0U ||
        static_cast<std::size_t>(token) > root_slots_.size()) {
        return nullptr;
    }
...
OpenSslTestKeyProvider::install_root(
    KeyProtectionBinding binding,
    RootKeyReference parent) noexcept {
...
OpenSslTestKeyProvider::acquire_system_root(KeyProtectionBinding system_binding) noexcept {
    ...
    for (std::size_t index = 0U; index < root_slots_.size(); ++index) {
        ...
        if (slot.binding != system_binding || slot.parent.valid()) {
...
OpenSslTestKeyProvider::acquire_child_root(
    ...
    if (parent_slot == nullptr || parent_slot->binding != parent_binding) {
        return key_error(errors::access_denied);
    }
    if (!valid_hierarchy_edge(parent_binding, child_binding)) {
        return key_error(errors::access_denied);
    }
...
OpenSslTestKeyProvider::generate_under_root(
    ...
    if (slot == nullptr || slot->binding != binding ||
...
os::core::Result<void>
OpenSslTestKeyProvider::destroy_root(
    RootKeyReference root,
    KeyProtectionBinding binding) noexcept {
    ...
    for (const auto& child : root_slots_) {
        if (child.occupied && child.parent == root) {
            return key_error(errors::access_denied);
        }
    }
