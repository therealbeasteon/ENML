...
- **M7.10 — the line count gate.** Done: `.github/scripts/kernel-line-count.sh`
  ...
  ceiling is the measured 4,453 lines, a ratchet rather than the 605-line
...
| discovery — FDT, inventory, GICv3 topology, timer discovery, boot memory | 1,217 |
...
wiring interrupt admission and the timer into the boot sequence. None of it
is discretionary — see `.github/scripts/kernel-line-count.sh` for the full
justification recorded beside each raise.
