...
`core/osui/platform/linux` now provides a concrete `LinuxTextBackend` behind the existing ENML interfaces.
...
The adapter owns a fixed five-role face table (`interface`, `display`, `international`, `symbols`, `monospace`), fixed-capacity UTF-16/break/segment scratch bounded by `SemanticText`, and pre-opened ICU analysis objects. It does not introduce a font scanner, per-text worker, polling loop or unbounded serialized paragraph.
...
`rasterize_opaque_frame_with_text()` composes the existing opaque material/contour path and this text path in one deterministic frame entry point. Geometry is painted first and text is painted afterward. The Linux backend is now exercised through this complete command-to-pixel path in CI rather than only through isolated shaping and mask calls.
...
The current Linux backend mutates its private font/analysis objects during shaping/rasterization and is therefore a renderer-owned serialized backend. ENML does not silently turn it into a globally concurrent font service; any future parallel renderer must either provide per-worker bounded backend state or explicit serialization after measurement shows that parallelism is justified.
...
M3.2 now has a concrete production-oriented Linux Unicode/font backend and a real semantic-command-to-pixel path. That does **not** mean the final ENML font product is finished.
