...
int main() {
    ...
    struct stat storage_transport_info {};
    if (::fstat(os::app::application_storage_service_fd, &storage_transport_info) != 0 ||
        !S_ISSOCK(storage_transport_info.st_mode)) {
        return 11;
    }

    auto storage_channel = os::ipc::Channel::adopt(
        os::core::NativeHandle{os::app::application_storage_service_fd});
    if (!storage_channel) return 12;
    ...
    // FD 5 is proven to be a connected IPC socket, not the application's
    // private-data directory. The first storage access therefore crosses the
    // identity-authenticated Storage Service and returns a typed root object
    // capability rather than exposing Linux filesystem authority directly.
