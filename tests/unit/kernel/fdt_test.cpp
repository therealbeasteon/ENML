...
int main() {
    ...
    // 40-byte header + 40-byte structure block + 11-byte strings block.
    std::array<std::byte, 91U> blob{};
    constexpr std::uint32_t structure_offset = 40U;
    constexpr std::uint32_t structure_size = 40U;
    constexpr std::uint32_t strings_offset = 80U;
    ...
    put_be32(blob.data() + 16U, 40U); // reservation map offset, not consumed yet
    ...
    cursor += 4U; // empty root name + padding
